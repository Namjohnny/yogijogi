...
public class Help {
	...
	public void botChat() {
		...
		while(true) {
			...
			System.out.println("0.고객센터 서비스 종료");
			int n = sc.nextInt();
			if(n==1) System.out.println("1.가입이 안돼요 2.등급이 따로 있나요?");
			else if(n==2) System.out.println("1.로그인이 안돼요 2.등급이 따로 있나요?");
			else if(n==3) System.out.println("1.예약확인이 안돼요 2.예약취소가 안돼요");
			else if(n==4) System.out.println("1.광고를 그만 보고 싶어요");
			else if(n==5) System.out.println("1.결제수단을 바꾸고 싶어요 2.결제확인은 어떻게 하나요?");
			else if(n==6) {
				System.out.println("어떤 문제인지 작성해주세요.");
				System.out.print(">>>");
				String claimMsg = sc.nextLine();
	...
	public void realChat() {
		new Commnunication(user).client();
	}
}
